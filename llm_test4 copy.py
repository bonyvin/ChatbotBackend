...
# async def extract_details(state: GraphState) -> Dict:
#     """
#     Extract invoice fields in the {value, is_example} format
#     """
...
#         print(f"--- Extracting details from: '{truncated_text}...' ---")
#         extracted = await extraction_chain.ainvoke({"text_to_extract": truncated_text})
        
#         # Check if we have a PO number to fetch details
#         if extracted.po_number and extracted.po_number.value:
#             print(f"--- Fetching PO details for: {extracted.po_number.value} ---")
#             po_details = await get_po_details(extracted.po_number.value)
            
#             if po_details:
#                 print(f"--- Found {len(po_details)} PO items ---")
#                 # Create items list from PO details as proper ExtractedInvoiceItem objects
#                 po_items = []
#                 for po_item in po_details:
#                     # Create a proper ExtractedInvoiceItem instance
#                     item = ExtractedInvoiceItem(
#                         item_id=ExtractedField(value=po_item.get('itemId'), is_example=False),
#                         quantity=ExtractedField(value=po_item.get('itemQuantity', 0), is_example=False),
#                         invoice_cost=ExtractedField(value=po_item.get('itemCost', 0.0), is_example=False)
#                     )
...
#                 # Set supplier ID from PO
#                 first_po_row = po_details[0]
#                 supplier_id_from_po = first_po_row.get('supplierId')
#                 if supplier_id_from_po:
#                     extracted.supplier_id = ExtractedField(value=supplier_id_from_po, is_example=False)

#         print("--- Extraction Successful ---")
#         print(f"Extracted Data: {extracted.model_dump_json(indent=2)}")
#         summary = f"""✅ Enriched PO {extracted.po_number.value}
#         - Supplier ID: {extracted.supplier_id.value}
#         - Items:
#         """ + "\n".join([
#             f"  • {item.item_id.value}"
...
# ) -> AsyncIterator[str]:
#     """
#     Asynchronously streams LLM response chunks and extracted details
#     to the client as soon as they arrive.
#     """
#     print("--- STREAM GENERATOR (for client) STARTED ---")
#     full_response_for_log = ""
...
#             elif event["event"] == "on_tool_end" and event["name"] == "extract_details":
#                 # Get the extracted details from the event
#                 tool_output = event["data"].get("output", {})
#                 extracted_details = tool_output.get("extracted_details")
                
#                 if extracted_details:
#                     # Format extracted details for streaming
#                     details_json = extracted_details.model_dump_json()
#                     yield f"\n\n--- EXTRACTION RESULTS ---\n{details_json}"
...
#     finally:
#         # If we haven't sent extracted details yet, send them now
#         if extracted_details and not any(
#             e["event"] == "on_tool_end" and e["name"] == "extract_details" 
#             for e in graph_stream
#         ):
#             details_json = extracted_details.model_dump_json()
#             yield f"\n\n--- EXTRACTION RESULTS ---\n{details_json}"
