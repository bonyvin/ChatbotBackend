...
from typing import Dict, List, TypedDict, Annotated, Sequence, AsyncIterator # Typing for state, tools, and streaming
import operator
...
# Graph node definitions remain the same
...
# --- Streaming Generator (Using stream_mode='messages') ---
async def stream_generator(app_runnable, input_state: dict, config: dict) -> AsyncIterator[str]:
    """
    Asynchronous generator that yields streamed content chunks from the LangGraph run.
    It specifically filters for the final LLM token streams using astream_events.
    """
    print("--- STREAM GENERATOR (events) STARTED ---") # Log start
    try:
        # Use astream_events to get granular events
        # version="v2" is recommended for newer LangChain/LangGraph versions
        async for event in app_runnable.astream_events(input_state, config, version="v2"):
            kind = event["event"]
            # Log all events received for debugging (optional)
            # print(f"--- Event Received: {kind}, Data: {event.get('data')} ---")

            # Check for events corresponding to LLM token streams
            if kind == "on_chat_model_stream":
                ...
                if chunk and hasattr(chunk, 'content'):
                    ...
                    if content_to_yield:
                        ...
                        print(f"--- BACKEND (events) YIELDING CHUNK: {content_to_yield!r} ---")
                        yield content_to_yield

            # You could potentially handle other event types here if needed
            # (e.g., tool start/end) by checking 'kind' and event['name']

    except Exception as e:
        print(f"!!! ERROR in stream_generator (events): {e} !!!")
        traceback.print_exc()
        yield f"\n\nStream error: {e}" # Yield an error message chunk
    finally:
         print("--- STREAM GENERATOR (events) FINISHED ---") # Log finish
# --- API Endpoint ---
@app.post("/chat/")
async def chat_endpoint(request: ChatRequest):
    ...
    try:
        ...
        return StreamingResponse(
            stream_generator(app_runnable, input_state, config),
